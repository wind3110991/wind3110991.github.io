...
#### （3）BSS段
BSS是Block Started by Symbol的简称，通常是指用来存放程序中未初始化的`全局变量`和`静态变量`。
...
a) 进程启动映射过程，并在虚拟地址空间中为映射创建Mapped Region——虚拟映射区域;  
b) 调用内核空间的系统调用函数mmap（不同于用户空间函数），实现文件物理地址和进程虚拟地址的一一映射关系;  
